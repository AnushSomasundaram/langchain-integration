#!/usr/bin/env python3
"""Packaging/build verification for `langchain-meetstream`: builds a real
...
from THAT installed copy, and confirms the import actually came from the
installed site-packages -- not this repo's source tree (which would happen
if the fresh venv somehow still saw the repo on `sys.path`, silently passing
even with a broken package).
...
_PACKAGES = [
    (_lib.LANGCHAIN_DIR, "langchain_meetstream", "MeetStreamLoader"),
]
