#!/usr/bin/env python3
"""Invoke MeetStream LangChain tools DIRECTLY, against a REAL bridge/real
MeetStream, with no LLM in the loop.

This is the key thing this script proves, separately from an agent demo:

    MeetStream API works
            v
        Bridge works
            v
        Tool works

...independent of "did an LLM correctly decide to call this tool." An agent
demo (see `examples/agent_example.py`) tests the LLM's decision-making; this
script tests the tool's own correctness with that variable removed.

Reads each tool's REAL input schema (`BaseTool.args`) rather than a hardcoded
parameter list -- if a tool's signature ever changes, `list` reflects that
automatically instead of silently going stale.

Usage:
    python scripts/verification/live_langchain_tools.py list
    python scripts/verification/live_langchain_tools.py invoke get_meeting --json '{"meeting_id": "abc123"}'
    python scripts/verification/live_langchain_tools.py invoke send_meeting_chat_message \\
        --json '{"meeting_id": "abc123", "message": "hello from verification"}'
"""
...
def _load_tools(bridge_url: str):
    ...
    return {t.name: t for t in get_meetstream_tools(client)}
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for name, tool in sorted(tools.items()):
        ...
        required = set(tool.args_schema.model_json_schema().get("required", [])) if tool.args_schema else set()
...
def cmd_invoke(args: argparse.Namespace) -> int:
    ...
    from langchain_meetstream import MeetStreamAPIError, MeetStreamConnectionError
