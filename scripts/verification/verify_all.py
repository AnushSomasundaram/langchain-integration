#!/usr/bin/env python3
"""Master verification runner for the MeetStream LangChain integration.
...
something works unless the corresponding script actually verified it.
    ...
    python scripts/verification/verify_all.py --local         # environment + bridge(static) + existing tests + ruff + langchain + git/security
...
def _run_local(start_bridge: bool) -> list[_lib.CheckResult]:
    ...
    results.extend(check_langchain.run(verbose=False))
...
def main() -> int:
    ...
    parser.add_argument("--local", action="store_true", help="Run only: environment, bridge (static), existing tests+ruff, LangChain, git/security")
