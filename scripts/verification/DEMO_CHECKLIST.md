...
- [ ] LangChain Loader verified: `python scripts/verification/live_langchain_test.py --bot-id <bot_id>` -> all PASS
- [ ] LangChain tools verified: `python scripts/verification/live_langchain_tools.py list` and at least one `invoke` against the real bridge
- [ ] LangChain agent verified: `python scripts/verification/live_langchain_agent.py --bot-id <bot_id>` -> all PASS (needs `OPENAI_API_KEY`)
- [ ] LangChain agent demo ready: `examples/agent_example.py` runs (needs `OPENAI_API_KEY`) -- run it once before the demo, not for the first time live
...
2. If live dispatch fails or a meeting won't cooperate during the actual
   ...
   transcript/loader/tools portions of the demo -- everything downstream of
   dispatch (`status`, `leave`, `transcript`, `live_langchain_test.py`) works
   identically against an older bot_id as a fresh one, since MeetStream
   retains the data.
