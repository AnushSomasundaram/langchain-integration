#!/usr/bin/env python3
"""REAL LangChain agent end-to-end test: a real tool-calling LLM decides on
...
This is the missing half of `live_langchain_tools.py`, which invokes tools
...
Prerequisites:
    - A running bridge with a real MEETSTREAM_API_KEY (see live_meetstream_test.py's docstring)
    - A --bot-id whose transcript is already processed (MeetStream-side)
    - OPENAI_API_KEY set, and `langchain`/`langchain-openai` installed
      (`pip install -e '.[examples]'`)

Usage:
    python scripts/verification/live_langchain_agent.py --bot-id <bot_id>
"""
...
def main() -> int:
    ...
    parser.add_argument("--model", default="gpt-4o-mini", help="Any langchain_openai ChatOpenAI-compatible model name")
    ...
    try:
        ...
        print(_line("langchain/langchain_openai import", "FAIL") + f" ({exc})")
        print(f"next step: pip install -e '{_lib.LANGCHAIN_DIR}[examples]'")
        return 1
    ...
    try:
        ...
        print(_line("langchain_meetstream import", "FAIL") + f" ({exc})")
    ...
    agent = create_agent(model=ChatOpenAI(model=args.model), tools=tools)
    ...
    tool_calls = [
        call for msg in messages for call in getattr(msg, "tool_calls", None) or [] if call["name"] == "get_meeting_transcript"
    ]
    if not tool_calls:
        print(_line("Agent called get_meeting_transcript", "FAIL") + " (model never invoked the tool)")
        print("next step: inspect the message transcript below -- the model may need a more explicit prompt")
        _print_transcript(messages)
        return 1
    print(_line("Agent called get_meeting_transcript", "PASS") + f" ({len(tool_calls)} call(s))")

    called_with_bot_id = any(call["args"].get("meeting_id") == args.bot_id for call in tool_calls)
    if not called_with_bot_id:
        print(_line("Agent passed the correct meeting_id", "FAIL") + f" (called with {[c['args'] for c in tool_calls]})")
        return 1
    print(_line("Agent passed the correct meeting_id", "PASS"))

    final_reply = (messages[-1].content or "").strip()
    if not final_reply:
...
def _print_transcript(messages: list) -> None:
    ...
    for msg in messages:
        ...
        print(f"  [{role}] {getattr(msg, 'content', msg)!r}")
