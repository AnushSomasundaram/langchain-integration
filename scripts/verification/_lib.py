...
it is never imported by `bridge/` or `langchain_meetstream/`. Every script in
this directory imports it the same way:
...
(`app.*`, `langchain_meetstream`) and inspects it directly, per this suite's
own ground rule of not re-implementing what it's trying to verify.
"""
...
def http_request_json(method: str, url: str, json_body: dict | None = None, timeout: float = 30.0) -> tuple[int, dict]:
    ...
    and deliberately does NOT import `langchain_meetstream` -- it verifies
