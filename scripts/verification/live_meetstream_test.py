...
`_lib.http_request_json`), not through `langchain_meetstream` -- this
script verifies the bridge + real MeetStream, a layer below the framework
package. See `live_langchain_test.py` for the framework-level real-data
check, which builds on a bot_id this script produces.
