...
```bash
git clone <repo-url> meetstream-langchain
cd meetstream-langchain
```
...
This repo has **two independent Python projects** (`bridge/` and this package, at the
repository root), each with its own `pyproject.toml`. Install each in editable mode with
its `dev` extra:
...
Add `[examples]` instead of/alongside `[dev]` on the package if you also want to run the
example scripts (pulls in `langchain`/`langchain-openai`).
...
```bash
python examples/loader_example.py <meeting_id>
```

## 8. Run the LangChain examples
...
a wheel + sdist in `dist/`. Verify a wheel installs cleanly in an isolated environment
before trusting it:

```bash
uv venv /tmp/verify-langchain-meetstream
/tmp/verify-langchain-meetstream/bin/pip install dist/*.whl
/tmp/verify-langchain-meetstream/bin/python -c "from langchain_meetstream import MeetStreamLoader; print('ok')"
```
...
ruff check langchain_meetstream tests examples
```

## Publishing (documented, not automated)

This repository does not auto-publish to PyPI. To publish the package manually once you
have PyPI credentials configured:
...
Bump `version` in `pyproject.toml` first. `.github/workflows/ci.yml` (if present) runs
install/lint/test/build on every push but does not upload anywhere — see that file's
comments for how you'd wire in a real publish step (a PyPI API token secret and a
`twine upload` step gated on a git tag) once this project is ready to publish for real.
