"""Tests for `langchain_meetstream.client.MeetStreamClient`."""
...
def test_reads_api_key_from_environment(monkeypatch):
    monkeypatch.setenv("MEETSTREAM_API_KEY", "from-env")
    client = MeetStreamClient(base_url=BRIDGE_URL)
...
def test_get_transcript(bridge_mock):
    ...
    assert transcript.meeting_id == "bot_1"
...
def test_send_chat_message(bridge_mock):
    ...
    result = client.send_chat_message("bot_1", "hello")
